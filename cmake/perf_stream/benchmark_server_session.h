...
typedef struct
{
     ...
     * in benchmark_server_session.c and run_elementwise_binary_once() in
     * benchmark_server_adapters.gen.c. Reuses output_offset/activation_min/activation_max
     * above (same semantics as convolve's output clamp). */
    ...
    int32_t out_mult;
    int32_t out_shift;
    /* ConvolutionFunctions DepthwiseConv scalar param -- see parse_scalar() in
     * benchmark_server_session.c and run_depthwise_conv_once() in
     * benchmark_server_adapters.gen.c. Reuses stride_h/w, pad_h/w, dilation_h/w,
     * output_h/w/c, input_offset, output_offset, activation_min/max above (same
     * semantics as convolve's). */
    int32_t ch_mult;
     ...
     * activation_min/max, output_h/w/c above (same semantics). See parse_scalar() in
     * benchmark_server_session.c and run_pooling_once() in benchmark_server_adapters.gen.c. */
    int32_t pool_h;
    int32_t pool_w;
     ...
     * convention as scale_bits -- see quant_scale_from_bits() in benchmark_server_adapters.gen.c). */
     ...
     * cases) -- see parse_scalar() in benchmark_server_session.c and run_convolve_once()
     * in benchmark_server_adapters.gen.c. Only meaningful for the F32/F16 dispatch path (S8/S16 select STANDARD vs packed via
     * a wholly separate kernel_id -- S4 -- rather than a runtime flag). */
    int32_t weight_format_is_packed;
    /* ActivationFunctions scalar params -- see parse_scalar() in benchmark_server_session.c
     * and run_activation_once() in benchmark_server_adapters.gen.c.
     * Relu/Relu6/Clamp/LeakyRelu/HardSwish* all reuse
    ...
    /* PReLU/PReLUScalar scalar params -- see parse_scalar() in benchmark_server_session.c
     * and run_prelu_once() in benchmark_server_adapters.gen.c. Reuses
     * input_offset/output_offset (own tensor's zero
    ...
    int32_t block_size;
    /* QuantizationFunctions Quantize/Dequantize scalar params -- see parse_scalar() in
     * benchmark_server_session.c and run_quantize_once()/run_dequantize_once() in
     * benchmark_server_adapters.gen.c. Reuses
    ...
    /* SoftmaxFunctions Softmax scalar params -- see parse_scalar() in
     * benchmark_server_session.c and run_softmax_once() in benchmark_server_adapters.gen.c.
     * Reuses out_mult/out_shift above for the kernel's
    ...
    int32_t row_size;
    int32_t diff_min;
    /* FullyConnectedFunctions FullyConnected scalar params -- see parse_scalar() in
     * benchmark_server_session.c and run_fully_connected_once() in
     * benchmark_server_adapters.gen.c. Reuses input_offset/
    ...
    /* FullyConnectedFunctions BatchMatMul scalar params -- see parse_scalar() in
     * benchmark_server_session.c and run_batch_matmul_once() in
     * benchmark_server_adapters.gen.c. Reuses input_offset (lhs
