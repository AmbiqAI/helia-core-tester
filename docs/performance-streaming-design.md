...
build id in `<build_dir>/hct_build_id.txt`, generated by
`scripts/generate_build_id.py` and advertised in HELLO; the skip path opens one
short RTT session to read it). Another build dir flashing the same probe, or no
HELLO at all, means a reflash. `--force` (or `hardware run --force-flash`)
overrides, and every stream also fails at HELLO if the board's build id is not
the build dir's:
