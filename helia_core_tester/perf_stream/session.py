...
class HostSession:
    ...
    def _check_counter_passes(self, target_info: TargetInfo) -> None:
        ...
        after TARGET_INFO/catalog; that is refused here, naming the passes and the limit.
...
def read_target_info(transport: Transport) -> TargetInfo:
    ...
    firmware sends TARGET_INFO on reset and then waits for TARGET_INFO_ACK, so a caller that
    just reads TARGET_INFO and closes leaves the board parked until the next
