...
class FakeTargetTransport:
    ...
    def _emit_target_info(self) -> None:
        """TARGET_INFO, field for field what hct_build_target_info_frame() emits, so the
        host decodes the fake with the real decode_target_info()."""
    ...
    def _admit_session_plan(self, payload: bytes) -> SessionPlan:
        ...
        max_cases_per_session cases and max_passes passes, no pass may need more PMU
        slots than the target has, and every case id must fit cursor_text()'s
        char[HCT_SERVER_MAX_CASE_ID] with its NUL."""
        ...
        # Same admission rule as handle_session_plan() in the firmware (HCT_SERVER_MAX_CASES).
        if not plan.cases or len(plan.cases) > self._max_cases_per_session:
            raise ValueError(f"SESSION_PLAN names {len(plan.cases)} cases; fake target accepts 1..{self._max_cases_per_session}.")
        # Same admission rule as parse_pmu_passes() in the firmware (HCT_SERVER_MAX_PASSES).
        if len(plan.passes) > self._max_passes:
            raise ValueError(f"SESSION_PLAN names {len(plan.passes)} PMU passes; fake target accepts at most {self._max_passes}.")
        for counter_pass in plan.passes:
            slots = counter_pass.slots_required
            if len(counter_pass.counters) > MAX_COUNTERS_PER_PASS or (self._pmu_present and slots > self._pmu_counter_slots):
        ...
        for case in plan.cases:
            ...
            if len(case.case_id.encode("utf-8")) + 1 > FAKE_MAX_CASE_ID:
