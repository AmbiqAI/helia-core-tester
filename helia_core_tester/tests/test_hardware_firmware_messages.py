...
from helia_core_tester.hardware.wire import CAP_PMU_ARMV8M, decode_kernel_catalog, decode_target_info
from helia_core_tester.hardware.hctp import HCTP_FLAG_MORE, FrameDecoder, MessageType
...
def test_firmware_target_info_and_catalog_roundtrip_with_python_decoder(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [
            ...
            str(PROJECT_ROOT / "cmake" / "hardware" / "benchmark_server_host_emit_main.c"),
            "-o",
            str(binary),
        ],
        check=True,
    ...
    canonical = json.dumps(
        json.loads((PROJECT_ROOT / "cmake" / "hardware" / "kernel_catalog.json").read_text()),
        sort_keys=True,
        separators=(",", ":"),
    ).encode("utf-8")
