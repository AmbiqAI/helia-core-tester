...
def test_path_layout_matches_config_helpers(tmp_path: Path) -> None:
    ...
    cpu = "cortex-m55"
    assert cfg.generated_tests_dir_for(cpu) == layout.generated_tests_dir(root, cpu, suite="int")
    assert layout.generated_tests_family_dir(root, cpu, "PoolingFunctions", suite="int") == root / "artifacts" / "generated_tests" / "int" / cpu / "PoolingFunctions"
    assert layout.generated_test_case_dir(root, cpu, "PoolingFunctions", "avg_pool_case_s8", suite="int") == root / "artifacts" / "generated_tests" / "int" / cpu / "PoolingFunctions" / "avg_pool_case_s8"
...
def test_find_build_dir_uses_canonical_layout(tmp_path: Path) -> None:
    ...
    assert resolved == layout.build_dir(tmp_path, cpu, compiler_tag="gcc", suite="int")
