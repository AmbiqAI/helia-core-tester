...
static struct {
    ...
    uint8_t body[DEPTHWISE_CONV_FLOAT_DEFAULT_F32_BUFFER_SIZE_MAX];
    uint8_t tail[HELIA_GUARD_BYTES];
} depthwise_conv_float_default_f32_buffer_guard;
#define depthwise_conv_float_default_f32_buffer (depthwise_conv_float_default_f32_buffer_guard.body)


#define DEPTHWISE_CONV_FLOAT_DEFAULT_F32_OUTPUT_SIZE (1 * 6 * 6 * 3)
static struct {
    uint8_t head[HELIA_GUARD_BYTES];
    float body[DEPTHWISE_CONV_FLOAT_DEFAULT_F32_OUTPUT_SIZE];
    uint8_t tail[HELIA_GUARD_BYTES];
} depthwise_conv_float_default_f32_output_guard;
#define depthwise_conv_float_default_f32_output (depthwise_conv_float_default_f32_output_guard.body)
...
{
    HELIA_GUARD_ARM(depthwise_conv_float_default_f32_output, false /* real output, not scratch: don't poison */);
    ...
    int failures = 0;
    HELIA_GUARD_CHECK(depthwise_conv_float_default_f32_buffer, "Depthwiseconv scratch", failures);
    HELIA_GUARD_CHECK_SLACK(depthwise_conv_float_default_f32_buffer, "Depthwiseconv scratch slack", depthwise_conv_float_default_f32_ctx.buf == depthwise_conv_float_default_f32_buffer ? (size_t)depthwise_conv_float_default_f32_ctx.size : 0u, failures);
    HELIA_GUARD_CHECK(depthwise_conv_float_default_f32_output, "Depthwiseconv output", failures);
