...
static int32_t convolve_float_default_f16_benchmark_run(void)
{
    ...
    const int32_t init_status = convolve_float_default_f16_bench_init();
    if (init_status != ARM_CMSIS_NN_SUCCESS) {
        HELIA_BENCH_PRINTF("[BENCH] convolve_float_default_f16 skipped: scratch sizer rejected before the context was populated\r\n");
        return helia_benchmark_failed("convolve_float_default_f16 benchmark init", init_status);
    }
    return helia_benchmark_run("convolve_float_default_f16", convolve_float_default_f16_bench_op);
}
#endif // HELIA_BENCHMARK_MODE
...
#ifdef HELIA_BENCHMARK_MODE
    int32_t failures = convolve_float_default_f16_benchmark_run();
