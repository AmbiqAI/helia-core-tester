...
def test_session_plan_over_target_rx_buffer_is_refused_before_sending(tmp_path: Path) -> None:
    ...
    from helia_core_tester.hardware.wire import encode_case_meta
...
def test_firmware_output_stream_is_pumped_after_outbox_drains() -> None:
    source = (PROJECT_ROOT / "cmake" / "hardware" / "benchmark_server_session.c").read_text()
...
def test_host_refuses_a_case_meta_over_the_advertised_receive_limit(tmp_path: Path) -> None:
    ...
    from helia_core_tester.hardware.session import case_meta_for_bundle, default_counter_passes, session_plan_for_bundles
    from helia_core_tester.hardware.wire import encode_case_meta, encode_session_plan
