...
class OpLogistic(OperationBase):
    ...
    def generate_c_files(self, output_dir: Path) -> None:
        ...
        h_content = self.render_template("ActivationFunctions/logistic/logistic.h.j2", context)
        h_path = includes_api_dir / f"{name}_logistic.h"
        with open(h_path, 'w') as f:
            f.write(h_content)
        
        c_content = self.render_template("ActivationFunctions/logistic/logistic.c.j2", context)
