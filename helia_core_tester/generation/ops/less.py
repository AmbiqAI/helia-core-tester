...
class OpLess(OperationBase):
    ...
    def convert_to_tflite(self, model, out_path: str, rep_seed: int) -> None:
        ...
        if activation_dtype == "S16":
            raise RuntimeError("Less S16 uses CMSIS-only generation; skip TFLite.")
        output_type = tf.int8
    ...
    def generate_c_files(self, output_dir) -> None:
        """
        Generate C and H files from templates for Less.
        """
        ...
        activation_dtype = self.desc.get('activation_dtype', 'S8')
        if activation_dtype == 'S16':
            kernel_fn = 'arm_less_s16'
            c_type = 'int16_t'
            np_in_dtype = np.int16
            qmin, qmax = -32768, 32767
        else:
            kernel_fn = 'arm_less_s8'
            c_type = 'int8_t'
            np_in_dtype = np.int8
            qmin, qmax = -128, 127
        ...
        if not tflite_path.exists():
            ...
            cmake_context = {
                'name': name,
                'operator': self.desc.get('operator', 'Less'),
