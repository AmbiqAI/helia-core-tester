...
class OpNotEqual(OperationBase):
    ...
    def convert_to_tflite(self, model, out_path: str, rep_seed: int) -> None:
        ...
        if activation_dtype == "S16":
            raise RuntimeError("NotEqual S16 uses CMSIS-only generation; skip TFLite.")
        output_type = tf.int8
    ...
    def generate_c_files(self, output_dir) -> None:
        """
        Generate C and H files from templates for NotEqual.
        """
        ...
        activation_dtype = self.desc.get('activation_dtype', 'S8')
        if activation_dtype == 'S16':
            kernel_fn = 'arm_not_equal_s16'
            c_type = 'int16_t'
            np_in_dtype = np.int16
            qmin, qmax = -32768, 32767
        else:
            kernel_fn = 'arm_not_equal_s8'
            c_type = 'int8_t'
            np_in_dtype = np.int8
            qmin, qmax = -128, 127
        ...
        if not tflite_path.exists():
            ...
            expected = simulate_compare(
                input_1_q,
                input_2_q,
                operation="ARM_COMPARE_NOT_EQUAL",
                input_1_offset=0,
            ...
            cmake_context = {
                'name': name,
                'operator': self.desc.get('operator', 'NotEqual'),
