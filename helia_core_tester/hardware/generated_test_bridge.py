...
produced by `helia_core_tester generate`) into hardware CaseBundles so they can
...
def _load_generation_sidecar(directory: Path) -> dict | None:
    """Load the structured `<name>_<suffix>.sidecar.json` written alongside a generated
    test's .c/.h source (see OperationBase._write_op_outputs()), if one exists.

    The sidecar is rendered from the *same* fully-resolved Jinja context used to produce
    the .c file, so its `scalars` dict is guaranteed in sync with the actual generated
    kernel call -- letting bridge builders read scalar call args structurally instead of
    regex-extracting them from generated C source via `_extract_call_args()`. Not every
    op-generator file emits a sidecar yet (only those routed through
    `_write_op_outputs()`); callers must fall back to regex extraction when this returns
    None.
    """
    candidates = sorted(directory.glob("*.sidecar.json"))
    if not candidates:
        return None
    import json
...
def _extract_bool_scalar(header_text: str, struct_name: str, field: str) -> bool:
    ...
    if struct_match is None:
        raise UnsupportedGeneratedTestError(f"Could not find struct `{struct_name}` in generated header")
    body = struct_match.group(1)
    field_pattern = re.compile(rf"\.{re.escape(field)}\s*=\s*(true|false)")
    field_match = field_pattern.search(body)
    if field_match is None:
        raise UnsupportedGeneratedTestError(f"Could not find field `.{field}` on struct `{struct_name}`")
    return field_match.group(1) == "true"
...
def build_case_bundle_from_generated_test(
    ...
    *,
    ...
    streamable hardware CaseBundle. Dispatches to a per-(family, operator) builder
    ...
    if builder is None:
        raise UnsupportedGeneratedTestError(
            ...
            f"hardware benchmark firmware dispatch yet (bridged today: "
...
def _build_convolve_case(
    ...
    *,
    ...
    streamable hardware CaseBundle, reusing the actual generator-produced input/weights/
    ...
    if activation_dtype == "S16":
        ...
        activation_numpy_dtype = np.float16 if activation_dtype == "FP16" else np.float32
        bias_numpy_dtype = np.float16 if activation_dtype == "FP16" else np.float32
        bias_wire_dtype = activation_dtype
    ...
    if input_flat.size < _shape_product(input_shape) or weights_flat.size < expected_weight_bytes:
        raise UnsupportedGeneratedTestError(
            ...
            f"test harness and hardware bridge both require at least the first dims-implied slice."
        )
    ...
    *,
    ...
    if operator != "NNActivationFloat" or input_dtype not in ("FP32", "FP16"):
        raise UnsupportedGeneratedTestError(
            ...
            "bridgeable -- hardware benchmark firmware only dispatches arm_nn_activation_f32/f16."
        )
...
def _build_reduce_sum_case(
    ...
    *,
    ...
    if operator != "ReduceSum" or input_dtype not in ("FP32", "FP16"):
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: operator={operator!r} input_dtype={input_dtype!r} is not "
            "bridgeable -- hardware benchmark firmware only dispatches arm_reduce_sum_f32/f16."
        )
...
def _build_batch_norm_case(
    ...
    *,
    ...
    if operator != "BatchNorm" or input_dtype not in ("FP32", "FP16"):
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: operator={operator!r} input_dtype={input_dtype!r} is not "
            "bridgeable -- hardware benchmark firmware only dispatches arm_batch_norm_f32/f16."
        )
...
def _build_depthwise_conv_case(
    ...
    *,
    ...
    - FP16 activation + FP16 weights -> arm_depthwise_conv_f16
    ...
    biases = (
        np.array(_extract_typed_array(header_text, f"{prefix}_biases", bias_wire_dtype), dtype=bias_numpy_dtype)
    ...
    is_float_dw = activation_dtype in ("FP32", "FP16")
    ...
    if weight_dtype == "S4":
        ...
        scratch_bytes = TemplateContextBuilder.calculate_depthwise_buffer_size_max(
            input_dims,
            filter_dims,
            output_dims,
            output_dtype="S16",
    ...
    if activation_dtype in ("S8", "S16"):
        arrays.extend([
            (next_blob_id, "multiplier", "S32", (output_channels,), multiplier, False, False),
            (next_blob_id + 1, "shift", "S32", (output_channels,), shift, False, False),
        ])
        next_blob_id += 2
    arrays.append((next_blob_id, "expected_output", activation_dtype, tuple(int(v) for v in expected_output.shape), expected_output, False, True))
    blobs: list[BlobInfo] = []
    ...
    manifest = {
        ...
        "serialized_scalar_parameters": {
            ...
            **({
                "float_activation_min_bits": _quant_scale_to_bits(activation_min),
                "float_activation_max_bits": _quant_scale_to_bits(activation_max),
            } if activation_dtype in ("FP32", "FP16") else {
                "activation_min": activation_min,
                "activation_max": activation_max,
            }),
            "ch_mult": ch_mult,
        ...
        "required_target_capabilities": [
            ...
            if weight_dtype == "S4"
                ...
                if activation_dtype == "S8"
                    ...
                    "depthwise_conv_s16"
                    if activation_dtype == "S16"
                    else ("depthwise_conv_f32" if activation_dtype == "FP32" else "depthwise_conv_f16")
                )
            )
        ],
        "repeated_invocation_safe": True,
...
def _build_pooling_case(
    ...
    *,
    ...
    into a streamable hardware CaseBundle. Unlike Convolve/DepthwiseConv, pooling has
    ...
    if operator not in ("AvgPool", "MaxPool") or activation_dtype not in ("S8", "S16"):
        raise UnsupportedGeneratedTestError(
            ...
            f"bridgeable -- hardware benchmark firmware only dispatches arm_avgpool_s8/s16 and "
    ...
    if input_dims["n"] != 1:
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: batch size {input_dims['n']} > 1 is not yet supported by the "
            f"hardware bridge (firmware dispatches a single pooling invocation per case)."
        )
    input_shape = (input_dims["n"], input_dims["h"], input_dims["w"], input_dims["c"])
    output_shape = (output_dims["n"], output_dims["h"], output_dims["w"], output_dims["c"])
...
def _build_pooling_float_case(
    ...
    *,
    ...
    hardware CaseBundle. Like the quantized pooling builder above, there are no weights/
    ...
    if operator not in ("AvgPool", "MaxPool") or input_dtype not in ("FP32", "FP16"):
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: operator={operator!r} input_dtype={input_dtype!r} is not "
            f"bridgeable -- hardware benchmark firmware only dispatches arm_avg/max_pool_f32/f16 "
...
def _build_activation_case(
    ...
    *,
    ...
    if (operator, activation_dtype) not in _ACTIVATION_CMSIS_FUNCTION:
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: operator={operator!r} activation_dtype={activation_dtype!r} is not "
            f"bridgeable -- hardware benchmark firmware only dispatches {sorted(_ACTIVATION_CMSIS_FUNCTION)}."
        )
...
def _build_quantize_case(
    ...
    *,
    ...
    if operator != "Quantize" or activation_dtype not in ("S8", "S16"):
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: operator={operator!r} activation_dtype={activation_dtype!r} is not "
            f"bridgeable -- hardware benchmark firmware only dispatches arm_quantize_f32_s8/s16."
        )
...
def _build_dequantize_case(
    ...
    *,
    ...
    if operator != "Dequantize" or activation_dtype not in ("S8", "S16"):
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: operator={operator!r} activation_dtype={activation_dtype!r} is not "
            f"bridgeable -- hardware benchmark firmware only dispatches arm_dequantize_s8_f32/s16_f32."
        )
    activation = str(descriptor.get("activation", "NONE"))
    if activation not in _ACTIVATION_KIND:
        raise UnsupportedGeneratedTestError(
...
def _build_requantize_case(
    ...
    *,
    ...
    if operator != "Requantize" or activation_dtype not in ("S8", "S16"):
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: operator={operator!r} activation_dtype={activation_dtype!r} is not "
            f"bridgeable -- hardware benchmark firmware only dispatches arm_requantize_s8_s8/s16_s16."
        )
...
def _build_prelu_case(
    ...
    *,
    ...
    if operator != "PReLU" or activation_dtype not in ("S8", "S16", "FP32", "FP16"):
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: operator={operator!r} activation_dtype={activation_dtype!r} is not "
            f"bridgeable -- hardware benchmark firmware only dispatches arm_prelu_s8/s16/f32/f16."
        )
...
def _build_prelu_scalar_case(
    ...
    *,
    ...
    if operator != "PReLUScalar" or activation_dtype not in ("S8", "S16"):
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: operator={operator!r} activation_dtype={activation_dtype!r} is not "
            f"bridgeable -- hardware benchmark firmware only dispatches arm_prelu_scalar_s8/s16."
        )
    header_path = _find_header_file(generated_test.directory)
    header_text = header_path.read_text(encoding="utf-8")
    source_path = _find_source_file(generated_test.directory)
...
def _build_softmax_case(
    ...
    *,
    ...
    if operator != "Softmax":
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: operator={operator!r} is not bridgeable -- hardware benchmark firmware "
...
def _build_abs_case(
    ...
    *,
    ...
    if operator != "Abs" or activation_dtype not in ("S8", "S16", "FP32", "FP16"):
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: operator={operator!r} activation_dtype={activation_dtype!r} is not "
            f"bridgeable -- hardware benchmark firmware only dispatches arm_abs_s8/s16/f32/f16."
        )
...
def _extract_elementwise_binary_tensors(
    ...
    *,
    ...
    if not allow_batch and (input1_dims["n"] != 1 or input2_dims["n"] != 1):
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: batch size > 1 is not yet supported by the hardware "
            f"hardware bridge (firmware dispatches a single {operator} invocation per case)."
        )
    ...
    *,
    ...
    if activation_dtype not in _ELEMENTWISE_BINARY_SUPPORTED_DTYPES:
        raise UnsupportedGeneratedTestError(
            ...
            f"hardware benchmark firmware only dispatches {_ELEMENTWISE_BINARY_SUPPORTED_DTYPES} {operator} kernels."
        )
    ...
    *,
    ...
    if operator != "FullyConnected" or (
        ...
        raise UnsupportedGeneratedTestError(
            ...
            f"activation_dtype={activation_dtype!r} is not bridgeable -- hardware benchmark firmware only "
...
def _build_batch_matmul_case(
    ...
    *,
    ...
    if operator != "BatchMatMul" or (
        (activation_dtype in ("S8", "S16") and weight_dtype != "S8")
        or (activation_dtype in ("FP32", "FP16") and weight_dtype != activation_dtype)
        or activation_dtype not in ("S8", "S16", "FP32", "FP16")
    ):
        raise UnsupportedGeneratedTestError(
            f"{generated_test.name}: operator={operator!r} weight_dtype={weight_dtype!r} "
            f"activation_dtype={activation_dtype!r} is not bridgeable -- hardware benchmark firmware only "
            f"dispatches arm_batch_matmul_s8/s16 and arm_batch_matmul_f32/f16."
        )
        ...
        for role, dims in (("input_lhs", input_lhs_dims), ("input_rhs", input_rhs_dims), ("output", output_dims)):
            if dims["n"] > 1 or dims["h"] > 1:
                raise UnsupportedGeneratedTestError(
                    ...
                    f"n={dims['n']}, h={dims['h']} are not yet supported by the hardware bridge."
                )
    ...
    input_lhs = _reshape_generated_prefix(
        np.array(_extract_typed_array(header_text, f"{prefix}_input_lhs", activation_dtype), dtype=activation_numpy_dtype),
        input_lhs_shape,
        generated_test=generated_test,
        tensor_name="input_lhs",
        context=f"input_lhs_shape={input_lhs_shape}",
    )
    input_rhs = _reshape_generated_prefix(
        np.array(_extract_typed_array(header_text, f"{prefix}_input_rhs", activation_dtype), dtype=activation_numpy_dtype),
        input_rhs_shape,
        generated_test=generated_test,
        tensor_name="input_rhs",
        context=f"input_rhs_shape={input_rhs_shape}",
    )
    expected_output = _reshape_generated_prefix(
        np.array(_extract_typed_array(header_text, f"{prefix}_expected_output", activation_dtype), dtype=activation_numpy_dtype),
        output_shape,
        generated_test=generated_test,
        tensor_name="expected_output",
        context=f"output_shape={output_shape}",
    ...
    manifest = {
        ...
        "serialized_scalar_parameters": {
            **({
                ...
                "out_mult": multiplier,
                "out_shift": shift,
            } if activation_dtype in ("S8", "S16") else {
                "float_activation_min_bits": _quant_scale_to_bits(activation_min),
                "float_activation_max_bits": _quant_scale_to_bits(activation_max),
                "adj_x": int(bool(adj_x)),
                "adj_y": int(bool(adj_y)),
            }),
            **({"output_n": output_dims["n"]} if output_dims["n"] != 1 else {}),
            **({"output_h": output_dims["h"]} if output_dims["h"] != 1 else {}),
            "output_w": output_dims["w"],
            "output_c": output_dims["c"],
        },
