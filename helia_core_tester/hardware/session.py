...
# Longest case id (UTF-8 bytes) a SESSION_PLAN may carry. The firmware stores each id in
...
class SessionResult:
    ...
    """Firmware build id the target advertised in TARGET_INFO (None for legacy callers)."""
...
def check_counter_passes(counter_passes: Sequence[CounterPass], info: TargetInfo) -> None:
    ...
    The pass list must fit the target's `max_passes` (the same bound the `--pmu-counters`
    parser applies up front from measurement.MAX_PASSES_PER_PLAN; here the advertised
    value is the authority). A DWT-only target (no
    ...
    if len(counter_passes) > limits.max_passes:
        names = ", ".join(counter_pass.name for counter_pass in counter_passes)
        raise RuntimeError(
            f"{len(counter_passes)} PMU passes planned ({names}) but target {info.board_id!r} runs at most "
            f"{limits.max_passes} per SESSION_PLAN (TARGET_INFO max_passes). Select fewer counters: each "
            f"group is measured in passes of up to {MAX_COUNTERS_PER_PASS} counters, so the pass count "
...
class HostSession:
    ...
    def handshake(self, *, expected_build_id: str | None = None) -> TargetInfo:
        ...
        If expected_build_id is given, the firmware build id advertised in TARGET_INFO
        must match it exactly, or the session fails before TARGET_INFO_ACK -- so a
        stream against firmware other than the one this host built (another build dir
        flashed the same probe, a stale board) never silently succeeds."""
...
def check_build_id(actual: str, expected: str | None) -> None:
    """Fail loudly when the firmware's TARGET_INFO build id is not the one the host expects."""
...
def read_target_info(transport: Transport) -> TargetInfo:
    """Read and decode the firmware's TARGET_INFO frame, sending nothing back.

    Used by the flash decision to ask the board which build it is running: the
    firmware sends TARGET_INFO on reset and then waits for TARGET_INFO_ACK, so a
    caller that just reads TARGET_INFO and closes leaves the board parked until
    the next reset-on-open session.
    """
...
def _compare_output_bytes(case_id: str, actual_output_bytes: bytes, bundle: CaseBundle) -> ComparisonResult:
    ...
    if actual_size != expected_size or len(actual_output_bytes) % itemsize != 0:
        ...
        print(
            f"[hardware] WARNING: case {case_id!r} output size mismatch -- "
