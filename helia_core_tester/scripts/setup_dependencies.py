...
The hardware firmware's own dependencies are not here: it is built as an NSX app,
so `nsx lock` / `nsx sync` fetch the SDK, board and kernel modules into the app
tree at the commits assets/dependency_baseline.json pins. ARM GCC and CMSIS-5 are
shared with the FVP path and stay.
...
Examples:
    ...
    # (CMSIS-5, Ethos-U core platform) install anywhere. Treat an unsupported OS
    # as "skip those two", not as a hard failure.
    try:
        get_architecture()
