    ...
    *,
    ...
    if descriptor_profile == "single_input_unary":
        return (
            ...
            + f"def {op_fn_name}(*, input_shape, dtype: str = \"int8\") -> bytes:\n"
            ...
            + "            dtype = \"int16\"\n"
            + "        else:\n"
            + f"            raise NotImplementedError(f\"Unsupported {operator} dtype: {{activation_dtype}}\")\n"
            + f"        model_bytes = {op_fn_name}(input_shape=tuple(self.desc[\"input_shape\"]), dtype=dtype)\n"
