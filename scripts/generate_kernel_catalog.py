...
def _render_catalog_c(entries: list[dict]) -> str:
    ...
    lines.append("}")
    lines.append("")
    lines.append("/* The firmware build defines HCT_BENCHMARK_SERVER_BUILD_ID_GENERATED and links a")
    lines.append(" * per-build hct_build_id.c (scripts/generate_build_id.py, run by CMake) instead;")
    lines.append(" * this constant only serves host-side unit builds of the protocol code. */")
    lines.append("#ifndef HCT_BENCHMARK_SERVER_BUILD_ID_GENERATED")
